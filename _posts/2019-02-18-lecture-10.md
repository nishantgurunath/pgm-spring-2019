...
This invloves matrix multiplication of $N_x x N_x$ matrix with another $N_x x N_x$ matrix, hence the time complexity is O(N_x^2)

Measurement Updates:
$K_{t+1} = P_{t+1|t}C^T(CP_{t+1|t}C^T + R)^{-1}$

This invloves a matrix inversion of $N_y x N_y$, hence the time complexity is O(N_y^3)

Overall time = max{N_x^2,N_y^3}
